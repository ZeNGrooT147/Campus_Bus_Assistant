...
const votingTopicsCache = new Map<string, {
  active: VotingTopic[],
  past: VotingTopic[],
  timestamp: number
}>();
...
export function useRealVotingTopics() {
    ...
    try {
      ...
      if (expiredTopics.length > 0) {
        ...
        const { error: updateError } = await supabase
          ...
          .in('id', expiredTopics.map(t => t.id));
      ...
      const validActiveTopics = activeTopics?.filter(topic => {
        const endDate = new Date(topic.end_date);
        return now <= endDate;
      }) || [];
      ...
      // Transform bus data
      const transformedBuses: Bus[] = busesData?.map(bus => ({
        id: bus.id,
      ...
      const validVotes = votesData?.filter(vote => {
        const voteTime = new Date(vote.created_at);
        const hoursSinceVote = (now.getTime() - voteTime.getTime()) / (1000 * 60 * 60);
        return hoursSinceVote <= VOTE_EXPIRY_HOURS;
      }) || [];

      // Delete expired votes
      const expiredVotes = votesData?.filter(vote => {
        const voteTime = new Date(vote.created_at);
        const hoursSinceVote = (now.getTime() - voteTime.getTime()) / (1000 * 60 * 60);
        return hoursSinceVote > VOTE_EXPIRY_HOURS;
      }) || [];
      ...
      const transformedActiveTopics = validActiveTopics?.map(topic => {
        ...
        const busNumber = transformedBuses.find(bus => bus.id === topic.bus_id)?.bus_number;
  ...
  const requestNewBus = useCallback(async (data: NewBusRequestData) => {
    ...
    try {
      ...
      const { data: selectedBus } = await supabase
        .from('buses')
        .select('*')
        .eq('id', data.busId)
        .single();
