...
const StudentVoting = () => {
  ...
  const { userRegionVotes, otherRegionVotes } = useMemo(() => {
    ...
    const otherVotes = votingTopics.filter(
      (vote) => vote.region !== userRegion
    );
  ...
  const VoteCard = React.memo(
    ({ vote }: { vote: (typeof votingTopics)[0] }) => {
        ...
        >
          <Card
            className={`overflow-hidden transition-all hover:shadow-md ${
              vote.hasVoted ? "border-primary/30" : ""
            }`}
          >
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between mb-1 text-sm">
                    <span>
                      Progress: {vote.votes.toFixed(1)}/{vote.requiredVotes}{" "}
                      votes
                    </span>
                    <span>
                    ...
                    {vote.votes > 0 && vote.votes < vote.requiredVotes && (
                      <motion.div
                        ...
                        style={{
                          width: `${Math.min(
                            100,
                            ((vote.votes + voteWeight) / vote.requiredVotes) *
                              100
                          )}%`,
                          opacity: vote.hasVoted ? 0 : 0.5,
                        }}
                        animate={{
                          opacity: vote.hasVoted ? 0 : [0.3, 0.6, 0.3],
                        }}
                ...
                {vote.busId && vote.busNumber && (
                  <div className="bg-primary/5 rounded-md p-2 flex items-center gap-2 text-sm">
                    ...
                    <span>
                      Requested Bus: <strong>#{vote.busNumber}</strong>
                    </span>
                  </div>
                )}
  ...
  const PastVoteCard = React.memo(
    ({ vote }: { vote: (typeof pastVotingTopics)[0] }) => (
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
          ...
          <CardHeader className="pb-2">
            <div className="flex justify-between">
              ...
              <div className="flex gap-2">
                ...
                <Badge
                  variant={
                    vote.status === "approved" ? "default" : "destructive"
                  }
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-1 text-sm">
                  <span>
                    Final Votes: {vote.votes}/{vote.requiredVotes}
                  </span>
                  <span>
                    {Math.round((vote.votes / vote.requiredVotes) * 100)}%
                  </span>
        ...
        <Card className="border-l-4 border-primary shadow-lg rounded-2xl bg-gradient-to-br from-primary/5 to-background dark:from-primary/10 dark:to-gray-800 mb-6 transition-all hover:shadow-xl">
          <CardContent className="flex flex-col gap-2 p-6">
            <div className="flex items-center gap-3 mb-2">
              ...
              <h2 className="text-xl font-bold flex items-center gap-1 text-foreground dark:text-white">
                ...
                <span className="relative group">
                  <svg
                    className="h-4 w-4 text-muted-foreground dark:text-gray-300 ml-1 cursor-pointer"
                    fill="none"
                    stroke="currentColor"
                    strokeWidth="2"
                    viewBox="0 0 24 24"
                  ...
                  <span className="absolute left-1/2 -translate-x-1/2 mt-2 w-48 bg-background dark:bg-gray-800 text-xs text-muted-foreground dark:text-gray-300 rounded shadow-lg p-2 opacity-0 group-hover:opacity-100 transition-opacity z-10 border border-border dark:border-gray-600">
                    This shows the number of active bus voting requests from
                    your region. Vote to help your request get approved!
                  </span>
                </span>
              </h2>
            </div>
            ...
            <div className="flex flex-col items-center justify-center my-2">
              <span className="text-5xl font-extrabold text-primary animate-pulse">
                {userRegionVotes.length}
              </span>
                ...
                <div className="w-full mt-2 mb-1">
                  <div className="h-2 rounded-full bg-primary/10 overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, userRegionVotes.length * 10)}%`,
                      }}
                    />
                  </div>
                </div>
              ...
              <span className="text-muted-foreground text-sm mt-2">
                ↗ Buses pending approval from your region
              </span>
            </div>
            <button
              ...
              onClick={() =>
                document
                  .getElementById("active-votes-section")
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center">
              ...
              <div className="flex-1">
                ...
                <p className="text-muted-foreground">
                  Vote for additional buses when you need them. Once enough
                  votes are received, a request is sent to the coordinator for
                  bus allocation.
                </p>
                <div className="flex items-center mt-2 text-sm font-medium">
                  ...
                  <Badge variant="outline" className="ml-2 px-2 py-0">
                    {userRegion === "Hubli"
                      ? "1.0× for Hubli, 0.5× for Dharwad votes"
                      : "1.0× for Dharwad, 0.5× for Hubli votes"}
                  </Badge>
                </div>
              </div>
              <div className="flex gap-2">
              ...
              <Card className="border border-dashed bg-muted/20">
                <CardContent className="flex flex-col items-center justify-center py-10">
                  ...
                  <h3 className="text-lg font-medium">
                    No active voting requests
                  </h3>
                  <p className="text-sm text-muted-foreground text-center max-w-md mt-2">
                    There are no active bus voting requests at the moment. You
                    can create a new request for additional buses.
                  </p>
                  <div className="mt-4">
                    <CreateVoteRequestDialog />
                  </div>
